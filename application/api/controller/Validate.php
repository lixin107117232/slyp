...
class Validate extends Api
{
     ...
     */
    public function check_paypwd_available()
    {
        $id = (int)$this->request->post('user_id');
        $pay_pwd = $this->request->post('pay_pwd');

        $count = User::where('id', $id)->field('pay_pwd,pay_salt')->find();
        if($count['pay_pwd'] == md5(md5($pay_pwd) . $count['pay_salt'])){
            $this->success("支付密码正确");
        }else{
            $this->error(__('支付密码不匹配'));
        }
    }
     ...
     */
    public function check_pwd_available()
    {
        $id = (int)$this->request->post('user_id');
        $password = $this->request->post('pwd');

        $count = User::where('id', $id)->field('password,salt')->find();
        if($count['password'] == md5(md5($password) . $count['salt'])){
            $this->success("登录密码正确");
        }else{
            $this->error(__('登录密码不匹配'));
        }
    }
